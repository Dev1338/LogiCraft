...
class ALUTab(QWidget):
    ...
    def _init_ui(self):
        ...
        def create_input_frame(title_text, is_dashed=False):
            ...
            title.setStyleSheet(
                "font-weight: 700; font-size: 11px; color: #48626e; letter-spacing: 0.5px;"
            )
            ...
            dec_lbl.setStyleSheet(
                "font-family: 'Courier New', monospace; font-size: 10px; font-weight: bold; color: #727783;"
            )
            val_dec = QLabel("0")
            val_dec.setStyleSheet(
                "font-family: 'Courier New', monospace; font-size: 11px; font-weight: bold; color: #1a1c1c;"
            )

            hex_lbl = QLabel("Hex:")
            hex_lbl.setStyleSheet(
                "font-family: 'Courier New', monospace; font-size: 10px; font-weight: bold; color: #ab47bc;"
            )
            val_hex = QLabel("0x0")
            val_hex.setStyleSheet(
                "font-family: 'Courier New', monospace; font-size: 11px; font-weight: bold; color: #ab47bc;"
            )
        ...
        # Input A
        self._input_a_group, a_lay, self._val_a_dec, self._val_a_hex = (
            create_input_frame("INPUT A", is_dashed=True)
        )
        ...
        self.input_a.value_changed.connect(
            lambda v: (
                self._val_a_dec.setText(str(v)),
                self._val_a_hex.setText(f"0x{v:X}"),
            )
        )
        ...
        # Input B
        self._input_b_group, b_lay, self._val_b_dec, self._val_b_hex = (
            create_input_frame("INPUT B", is_dashed=False)
        )
        ...
        self.input_b.value_changed.connect(
            lambda v: (
                self._val_b_dec.setText(str(v)),
                self._val_b_hex.setText(f"0x{v:X}"),
            )
        )
        ...
        op_label.setStyleSheet(
            "font-weight: 700; font-size: 11px; color: #48626e; letter-spacing: 0.5px; margin-top: 8px;"
        )
        ...
        for cat, ops in OPERATIONS.items():
            for op in ops:
                self.op_combo.addItem(
                    f"{cat}: {op}" if cat != "" else op, op
                )  # Adjust to format nicely
        ...
        self.flag_v = FlagPill("OVF", is_error=True)
        ...
        def create_res_row(label):
            ...
            lbl = QLabel(label)
            lbl.setStyleSheet(
                "font-family: 'Courier New', monospace; font-size: 11px; color: #1a1c1c; font-weight: normal;"
            )
            val = DraggableValueLabel("—")
            val.setStyleSheet(
                "font-family: 'Courier New', monospace; font-size: 11px; color: #1a1c1c; font-weight: bold;"
            )
            row.addWidget(lbl)
            row.addStretch()
            row.addWidget(val)
        ...
        self._res_signed.setStyleSheet(
            "font-family: 'Courier New', monospace; font-size: 11px; color: #ba1a1a; font-weight: bold;"
        )
    ...
    def _compute(self):
        ...
        # History
        entry = f"A={a} {op} B={b} = {res.result} [Z={int(res.zero)} C={
            int(res.carry)
        } V={int(res.overflow)} S={int(res.sign)}]"
    ...
    def _draw_carry_chain(self, res: ALUResult):
        ...
        for i in range(n):
            ...
            ax.add_patch(
                __import__(
                    ...
                    facecolor=p.surface_container_lowest,
        ...
        ax.axis("off")
        title = (
            f"{self._bw}-Bit {res.gate_labels[0]} Chain"
            if res.gate_labels
            else "Result"
        )
