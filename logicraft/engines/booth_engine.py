...
def run_booth(m, r, bit_width):
    ...
    cycles.append(BoothCycleState(
        cycle=0, a_reg=a, q_reg=q, q_minus_1=q_minus_1,
        action="INIT", action_reason="Initial state",
        a_bits=_bits(a, bit_width), q_bits=_bits(q, bit_width),
        a_signed=_to_signed(a, bit_width), q_signed=_to_signed(q, bit_width),
    ))
    ...
    for cycle in range(1, n + 1):
        ...
        if q0 == 0 and q_minus_1 == 1:
            action = "ADD M"
            reason = f"Q0=0, Q-1=1 -> A = A + M"
            a = (a + m_val) & mask
        elif q0 == 1 and q_minus_1 == 0:
            action = "SUB M"
            reason = f"Q0=1, Q-1=0 -> A = A - M"
        ...
        cycles.append(BoothCycleState(
            cycle=cycle, a_reg=a, q_reg=q, q_minus_1=q_minus_1,
    ...
    product_bits_str = format(_to_unsigned(product_signed, 2 * bit_width), f"0{2 * bit_width}b")

    return BoothResult(
        multiplicand=m_signed, multiplier=_to_signed(r, bit_width),
        bit_width=bit_width, cycles=cycles,
        final_product=product_signed, expected_product=expected,
